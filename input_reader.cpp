...
std::string_view TrimWhitespace(std::string_view text) {
    ...
    return start == text.npos ? "" : text.substr(start, text.find_last_not_of(' ') + 1 - start);
}
...
std::vector<std::string_view> SplitByDelimiter(std::string_view text, char delimiter) {
    ...
    while ((current_pos = text.find_first_not_of(' ', current_pos))) {
        if (current_pos == text.npos) break;
        ...
        current_pos = delim_pos == text.npos ? text.size() : delim_pos + 1;
    }
...
std::vector<std::string_view> ProcessRoute(std::string_view route_desc) {
    const bool is_circular = route_desc.find('>') != route_desc.npos;
